...
void spin_iters(unsigned int x) __attribute__((optimize("O0")));
unsigned int cycs_per_usec = CYCS_PER_USEC;

void
spin_iters(unsigned int x)
{
	unsigned int i;
...
spin_calib(void)
{
	...
	printf("Spin calibration: ITERS:%u Cycs/ITERS:%llu usecs/ITERS:%llu\n", 
