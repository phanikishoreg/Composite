...
start32:
  ...
  call    bootmain
    7c48:	e8 dc 00 00 00       	call   7d29 <bootmain>
...
outb(ushort port, uchar data)
{
  ...
  // Read data.
  waitdisk();
    7cc9:	e8 b0 ff ff ff       	call   7c7e <waitdisk>
}
...
readseg(uchar* pa, uint count, uint offset)
{
    ...
    7ce3:	56                   	push   %esi
    7ce4:	53                   	push   %ebx
    7ce5:	83 ec 08             	sub    $0x8,%esp
    7ce8:	8b 5c 24 18          	mov    0x18(%esp),%ebx
    7cec:	8b 74 24 20          	mov    0x20(%esp),%esi
  uchar* epa;

  epa = pa + count;
    7cf0:	89 df                	mov    %ebx,%edi
    7cf2:	03 7c 24 1c          	add    0x1c(%esp),%edi

  // Round down to sector boundary.
  pa -= offset % SECTSIZE;
    7cf6:	89 f0                	mov    %esi,%eax
    7cf8:	25 ff 01 00 00       	and    $0x1ff,%eax
    7cfd:	29 c3                	sub    %eax,%ebx

  // Translate from bytes to sectors; kernel starts at sector 1.
  offset = (offset / SECTSIZE) + 1;
    7cff:	c1 ee 09             	shr    $0x9,%esi
    7d02:	83 c6 01             	add    $0x1,%esi

  // If this is too slow, we could read lots of sectors at a time.
  // We'd write more to memory than asked, but it doesn't matter --
  // we load in increasing order.
  for(; pa < epa; pa += SECTSIZE, offset++)
    7d05:	39 df                	cmp    %ebx,%edi
    7d07:	76 19                	jbe    7d22 <readseg+0x40>
    readsect(pa, offset);
    7d09:	89 74 24 04          	mov    %esi,0x4(%esp)
    7d0d:	89 1c 24             	mov    %ebx,(%esp)
    7d10:	e8 78 ff ff ff       	call   7c8d <readsect>
  offset = (offset / SECTSIZE) + 1;
  ...
  for(; pa < epa; pa += SECTSIZE, offset++)
    ...
    7d20:	77 e7                	ja     7d09 <readseg+0x27>
    readsect(pa, offset);
}
...
bootmain(void)
{
  ...
  readseg((uchar*)elf, 4096, 0);
    ...
    7d47:	e8 96 ff ff ff       	call   7ce2 <readseg>

  // Is this an ELF executable?
  if(elf->magic != ELF_MAGIC)
    7d4c:	81 3d 00 00 01 00 7f 	cmpl   $0x464c457f,0x10000
    7d53:	45 4c 46 
    7d56:	75 59                	jne    7db1 <bootmain+0x88>
  ...
  for(; ph < eph; ph++){
    ...
    7d71:	73 38                	jae    7dab <bootmain+0x82>
}
...
stosb(void *addr, int data, int cnt)
{
  asm volatile("cld; rep stosb" :
    ...
    pa = (uchar*)ph->paddr;
    7d78:	8b 7b 0c             	mov    0xc(%ebx),%edi
    readseg(pa, ph->filesz, ph->off);
    7d7b:	8b 43 04             	mov    0x4(%ebx),%eax
    7d7e:	89 44 24 08          	mov    %eax,0x8(%esp)
    7d82:	8b 43 10             	mov    0x10(%ebx),%eax
    7d85:	89 44 24 04          	mov    %eax,0x4(%esp)
    7d89:	89 3c 24             	mov    %edi,(%esp)
    7d8c:	e8 51 ff ff ff       	call   7ce2 <readseg>
    if(ph->memsz > ph->filesz)
    7d91:	8b 4b 14             	mov    0x14(%ebx),%ecx
    7d94:	8b 43 10             	mov    0x10(%ebx),%eax
    7d97:	39 c1                	cmp    %eax,%ecx
    7d99:	76 09                	jbe    7da4 <bootmain+0x7b>
      stosb(pa + ph->filesz, 0, ph->memsz - ph->filesz);
    7d9b:	01 c7                	add    %eax,%edi
    7d9d:	29 c1                	sub    %eax,%ecx
    7d9f:	89 e8                	mov    %ebp,%eax
    7da1:	fc                   	cld    
    7da2:	f3 aa                	rep stos %al,%es:(%edi)
    return;  // let bootasm.S handle error
  ...
  eph = ph + elf->phnum;
  for(; ph < eph; ph++){
    7da4:	83 c3 20             	add    $0x20,%ebx
    7da7:	39 de                	cmp    %ebx,%esi
    7da9:	77 cd                	ja     7d78 <bootmain+0x4f>
  }
  ...
  entry();
    7dab:	ff 15 18 00 01 00    	call   *0x10018
}
    7db1:	83 c4 1c             	add    $0x1c,%esp
    7db4:	5b                   	pop    %ebx
    7db5:	5e                   	pop    %esi
    7db6:	5f                   	pop    %edi
    7db7:	5d                   	pop    %ebp
