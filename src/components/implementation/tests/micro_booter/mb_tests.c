...
#define TIMEOUT_CYCS 100000
int hpet_overhead_test = 0;
static void
thd_fn_perf(void *d)
{
	cycles_t now;
	...
	cos_switch(BOOT_CAPTBL_SELF_INITTHD_BASE, BOOT_CAPTBL_SELF_INITTCAP_BASE, TCAP_PRIO_MAX, hpet_overhead_test ? timer : TCAP_TIME_NIL, BOOT_CAPTBL_SELF_INITRCV_BASE);
...
static void
test_thds_perf(int with_hpet_prog)
{
	thdcap_t ts;
	...
	if (with_hpet_prog) hpet_overhead_test = 1;
	else hpet_overhead_test = 0;

	rdtscll(now);
	timer = tcap_cyc2time(now + TIMEOUT_CYCS * cyc_per_usec);
	cos_switch(ts, BOOT_CAPTBL_SELF_INITTCAP_BASE, TCAP_PRIO_MAX, hpet_overhead_test ? timer : TCAP_TIME_NIL, BOOT_CAPTBL_SELF_INITRCV_BASE);
...
//	test_thds();
	test_thds_perf(0); /* w/o HPET first */
	test_thds_perf(1); /* w/ HPET */
